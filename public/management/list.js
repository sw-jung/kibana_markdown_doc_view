...
uiRoutes
    .when('/management/kibana/markdown_template', {
        template,
        controller: ($scope, $route, markdownTemplates, confirmModal, es, kbnIndex, Notifier, Private) => {
                ...
                return Promise.all([
                    ...
                    markdownTemplates.list()
                        ...
                        $scope.list = chain(indexPatterns)
                            ...
                            .reduce((map, hit) => {
                                if (hit) {
                                    if (hit.type === 'markdown_template'){
                                        hit._id = hit.id;
                                        hit._type = 'markdown_template';
                                        hit._source = { "markdown_template": hit.attributes};
                                    }
